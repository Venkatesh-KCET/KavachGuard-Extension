...
console.log("Profanity Blocker * V1.5 * is ready.");
...
function addWebStatistics(site, profanityCount){
	...
	chrome.storage.local.get(['websites'],function(result){
		...
		chrome.storage.local.set({websites},function(){  
			...
			console.log("Array (website stats) pushed.")
		});
	});
}

//Check if website is a warning domain
	...
	chrome.storage.local.get(['warningDomains'],function(result){
		for(var i = 0; i < result.warningDomains.length; i++){
			if(origin_site === result.warningDomains[i]){
				console.log("WARNING: Domain blacklisted")
...
function toggleFilter(){
	if(filterToggle === true){ // if main toggle is enabled, enable filters.
